...
##########################################################################
# Get voltage source calibration table (for range selected above.)

for index in range (0, 5):

	timeout = 1.0
	i, dac, voltage, timeout = \
		libxsmu.VS_getCalibration (deviceID, index, timeout)
	print i, dac, voltage, timeout

	if (timeout == 0.0):
		print 'Communication timeout in VS_getCalibration'
		exit (-2)
